...
function Sales ({ data }) {
  return (
    <div className={styles.sales}>
      <div className={styles.title}>交易金额趋势</div>
      <ResponsiveContainer minHeight={360}>
        <LineChart data={data}>
          ...
          />
          <Line type="monotone" dataKey="Amount" stroke={color.purple} strokeWidth={3} dot={{ fill: color.purple }} activeDot={{ r: 5, strokeWidth: 0 }} />
          {/*<Line type="monotone" dataKey="交易笔数" stroke={color.red} strokeWidth={3} dot={{ fill: color.red }} activeDot={{ r: 5, strokeWidth: 0 }} />*/}
        </LineChart>
      </ResponsiveContainer>
    </div>
  )
